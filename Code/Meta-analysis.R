...
# metafor        dplyr clubSandwich     openxlsx 
# "4.8.0"      "1.1.4"      "0.6.2"    "4.2.8.1" 


## 1.2 File paths ----
ROOT     <- "J:/3rd project_SMR/Systematic review/Final review/05_meta-analysis_R"
XLSX_IN  <- file.path(ROOT, "data_in/SMR_T2DM_input_final.xlsx")
...
# Some spreadsheet cells contain XML character references (e.g. "&amp;", "&#241;",
# "&gt;=") that openxlsx leaves undecoded on non-UTF-8 system locales (e.g. GBK); a few
# cells are also double-encoded at source. Decode them so every label renders as real
# text ("&", "n-tilde", ">=", ...) regardless of the machine's locale.
decode_xml_entities <- function(x) {
  ...
  regmatches(x, m) <- lapply(regmatches(x, m), function(codes) {
    vapply(codes, function(cc) {
      ...
      n <- if (grepl("^[xX]", body)) strtoi(substring(body, 2L), 16L) else suppressWarnings(as.integer(body))
...
# Fold accented Latin letters (and en/em dashes, sharp s) to plain ASCII so forest
# labels render identically on every machine, even if the input is regenerated with
# accented characters. Built from code points, so this source stays pure ASCII.
...
# The first workbook tab has one paper per row, with separate columns for
# overall/both-sex, male, and female SMRs. Sex-specific analyses are disabled
# for now, so only the overall/both-sex rows are used in the active analysis.

## 3.1 Paper-level rows (both, male, female) ----
paper_both <- paper_overall |>
  transmute(
    estimate_id  = paste0(StudyID, "_overall_both"),
    ...
    period_label = ifelse(
      ...
      ifelse(!is.na(Study_Period_Raw) & Study_Period_Raw != "NR",
             as.character(Study_Period_Raw),
             "period unknown")
    ),
    period_start = as.numeric(Study_Period_Start_Year),
    period_end   = as.numeric(Study_Period_End_Year),
    sex          = "Both",
    cause        = "all-cause",
    SMR          = as.numeric(SMR_Overall),
    SMR_LCI      = as.numeric(SMR_Overall_LCI),
    SMR_UCI      = as.numeric(SMR_Overall_UCI),
    N_deaths     = as.numeric(N_Deaths_Total),
    N_subjects   = suppressWarnings(as.numeric(N_Subjects_Total))
  )

paper_male <- paper_overall |>
  transmute(
    estimate_id  = paste0(StudyID, "_overall_male"),
    source       = "paper-overall",
    StudyID      = as.character(StudyID),
    First_Author_Lastname,
    cohort_id,
    ...
    sex          = "Male",
    cause        = "all-cause",
    SMR          = as.numeric(SMR_Male),
    SMR_LCI      = as.numeric(SMR_Male_LCI),
    SMR_UCI      = as.numeric(SMR_Male_UCI),
    N_deaths     = as.numeric(N_Deaths_Male),
    N_subjects   = suppressWarnings(as.numeric(N_Subjects_Male))
  )

paper_female <- paper_overall |>
  transmute(
    estimate_id  = paste0(StudyID, "_overall_female"),
    source       = "paper-overall",
    StudyID      = as.character(StudyID),
    First_Author_Lastname,
    cohort_id,
    ...
    sex          = "Female",
    cause        = "all-cause",
    SMR          = as.numeric(SMR_Female),
    SMR_LCI      = as.numeric(SMR_Female_LCI),
    SMR_UCI      = as.numeric(SMR_Female_UCI),
    N_deaths     = as.numeric(N_Deaths_Female),
    N_subjects   = suppressWarnings(as.numeric(N_Subjects_Female))
  )

paper_long <- paper_both
# To re-enable sex-specific analyses later, replace the line above with:
# paper_long <- bind_rows(paper_both, paper_male, paper_female)

## 3.2 Sub-period rows ----
# Add the calendar sub-period estimates from the second workbook tab.
# Rows labelled as "overall" / "whole period" / "full follow-up" are skipped
# to avoid duplicating the paper-level overall estimate from tab 1.
paper_meta <- paper_overall |>
  mutate(StudyID = as.character(StudyID)) |>
  select(StudyID, OECD, Setting_Type, Year,
         N_Subjects_Total, N_Subjects_Male, N_Subjects_Female)

sub_long <- sub_period |>
  mutate(StudyID = as.character(StudyID)) |>
  filter(!grepl("overall|whole period|whole-period|full follow",
                tolower(as.character(period_label))),
         # Keep only all-ages, both-sex calendar sub-periods. Drop age/sex-restricted
         # sub-strata (e.g. Sasaki's age<65 "lt65" rows): these overlap the all-ages
         # estimate for the same period and would double-count that person-time if a CI
         # were ever supplied for them. The marker lives in estimate_id, not period_label.
         !grepl("lt[0-9]|ge[0-9]|_age|_male$|_female$|_men$|_women$",
                tolower(as.character(estimate_id)))) |>
  mutate(
    SMR      = as.numeric(SMR),
    SMR_LCI  = as.numeric(SMR_LCI),
    SMR_UCI  = as.numeric(SMR_UCI),
    N_deaths = as.numeric(N_deaths),
    sex = ifelse(
      sex %in% c("Both", "All", "Total"),
      "Both",
      ifelse(grepl("^[Mm]", sex), "Male",
             ifelse(grepl("^[FfWw]", sex), "Female", "Both"))
    ),
    period_start = suppressWarnings(as.numeric(sub("^\\D*([0-9]{4}).*$", "\\1", period_label))),
    period_end   = suppressWarnings(as.numeric(sub("^.*?-\\s*([0-9]{4}).*$", "\\1", period_label)))
  ) |>
  filter(sex == "Both") |>
  left_join(paper_meta, by = "StudyID") |>
  mutate(
    N_subjects = case_when(
      sex == "Male"   ~ suppressWarnings(as.numeric(N_Subjects_Male)),
      sex == "Female" ~ suppressWarnings(as.numeric(N_Subjects_Female)),
      TRUE            ~ suppressWarnings(as.numeric(N_Subjects_Total))
    )
  ) |>
  select(
    estimate_id, source, StudyID, First_Author_Lastname, cohort_id,
    Country, WHO_region, WB_HIC, OECD, Setting_Type, Year,
    period_label, period_start, period_end,
    sex, cause, SMR, SMR_LCI, SMR_UCI, N_deaths, N_subjects
  )
...
# Added BEFORE any pool is fit, so all res_* objects carry these columns
# and the forest plots can display them as extra columns.
long <- long |>
  mutate(
...
cat("Built long-format data:",
    ...
    sum(long$source == "sub-period"), "sub-period =",
    nrow(long), "rows\n")
...
fit_pool <- function(data, stratum_label) {
  ...
  empty_summary <- data.frame(
    ...
    SMR = NA_real_,
    SMR_LCI_naive = NA_real_, SMR_UCI_naive = NA_real_,
    SMR_LCI_robust = NA_real_, SMR_UCI_robust = NA_real_,
    sigma2_cohort = NA_real_, sigma2_study = NA_real_, sigma2_estimate = NA_real_,
    PI_LCI = NA_real_, PI_UCI = NA_real_,
  ...
  summary_row <- data.frame(
    ...
    PI_LCI = as.numeric(pred$pi.lb),
    PI_UCI = as.numeric(pred$pi.ub),
...
## 4.2 draw_forest() - study-level forest plot ----
# Parameters:
#   sort_by_period - FALSE = sort by Year/Author (default); TRUE = sort by
...
draw_forest <- function(pool, title_text, file_stub,
                        sort_by_period = FALSE,
                        show_hic = FALSE,
                        show_ukpds = FALSE) {
  if (is.null(pool$model)) return(invisible(NULL))
  ...
  row_order <- if (isTRUE(sort_by_period)) {
    ...
    order(data$Year, data$First_Author_Lastname,
          data$StudyID, data$period_label)
  }
  data <- data[row_order, ]
  ...
  shorten_text <- function(x, max_chars) {
    ifelse(nchar(x) > max_chars,
           paste0(substr(x, 1, max_chars - 3), "..."), x)
  }
  ...
  # ---- Cell values ----
  author_year <- paste0(shorten_text(data$First_Author_Lastname, 12),
                        ", ", data$Year)
  death_n <- ifelse(
  ...
  # Deaths / N total: count each study once. Calendar sub-period rows carry
  # the study-level N_subjects (joined from tab 1), so a plain row-sum
  # multiply-counts any cohort that contributes more than one row.
  study_tot <- data |>
    group_by(StudyID) |>
    summarise(
  ...
  total_death_n  <- paste0(fmt_count(total_deaths), " / ",
                           fmt_count(total_subjects))
  ...
  heterogeneity_text <- sprintf(
    "Test for heterogeneity: QE = %.2f, p %s",
    model$QE, fmt_p(model$QEp)
  )
  ...
  # ---- Drawing closure (used for both screen/PDF and PNG copy) ----
  draw_one <- function() {
    old_par <- par(no.readonly = TRUE)
    layout(matrix(c(1, 2, 3), nrow = 1), widths = layout_widths)
    par(oma = c(1.2, 0, 2.2, 0))
    
    # Left table - study details
  ...
  # PDF page (current device) + PNG copy
  draw_one()
  
  png_path <- file.path(FIG_DIR, paste0(file_stub, ".png"))
  png(png_path, width = 3000,
      height = max(1050, 42 * nrow(data) + 420), res = 220)
  draw_one()
  dev.off()
...
draw_combined_subgroup_plot <- function(subgroup_all) {
  ...
  for (plot_copy in c("pdf_page", "png_file")) {
    if (plot_copy == "png_file") {
      png(file.path(FIG_DIR, "fig_subgroups_all.png"),
          width = 3200,
          height = max(1400, 68 * nrow(subgroup_display) + 420), res = 220)
    }
    ...
    text(x = x_left_text,  y = y_header, "Subgroup level", adj = 1,
         font = 2, cex = 0.75)
    text(x = x_right_text, y = y_header, "SMR (95% CI)",   adj = 0,
         font = 2, cex = 0.75)
    ...
    for (j in seq_len(nrow(subgroup_display))) {
      ...
      if (this_row$row_type == "header") {
        ...
        segments(x0 = x_min, x1 = x_max,
                 y0 = this_row$y - 0.26, y1 = this_row$y - 0.26,
                 col = "grey85")
      } else {
        left_label <- sprintf("  %s (k=%d, cohorts=%d)",
                              this_row$Group,
                              this_row$k_estimates, this_row$k_cohorts)
        text(x = x_left_text, y = this_row$y, labels = left_label,
             adj = 1, cex = 0.62)
        ...
        if (is.na(this_row$SMR)) {
          ...
          if (this_row$SMR_LCI < x_min) {
            arrows(x0 = x_min * 1.15, y0 = this_row$y,
                   x1 = x_min,        y1 = this_row$y,
                   length = 0.05, lwd = 2, col = "#0B2C4D")
          }
          if (this_row$SMR_UCI > x_max) {
            arrows(x0 = x_max / 1.15, y0 = this_row$y,
                   x1 = x_max,        y1 = this_row$y,
                   length = 0.05, lwd = 2, col = "#0B2C4D")
          }
          points(this_row$SMR, this_row$y, pch = 18, cex = 1.05,
                 col = "#C4373A")
...
d_overall <- long |> filter(sex == "Both", cause == "all-cause")
...
## 5.2 HIC vs Non-HIC ----
res_hic <- fit_pool(
  long |> filter(HIC_grp == "HIC",      sex == "Both", cause == "all-cause"),
  "HIC: overall"
)
res_nonhic <- fit_pool(
  long |> filter(HIC_grp == "Non-HIC",  sex == "Both", cause == "all-cause"),
  "Non-HIC: overall"
)
...
res_ukpds <- fit_pool(
  ukpds_overlap |> filter(sex == "Both", cause == "all-cause"),
  "UKPDS-era overlap: overall"
)
res_hic_ukpds <- fit_pool(
  ukpds_overlap |> filter(HIC_grp == "HIC", sex == "Both", cause == "all-cause"),
...
subgroup_all <- bind_rows(
  subgroup_all,
  build_subgroup_rows(
    long |> filter(sex == "Both", cause == "all-cause"),
    "ukpds_overlap_grp",
...
readme <- data.frame(
  Item = c("Input", "Analysis", "Primary estimate", "Primary CI",
  ...
  Value = c(
    ...
    sprintf("%.2f from %d estimates, %d studies, %d cohorts",
            res_overall$summary$SMR,
            res_overall$summary$k_estimates,
            res_overall$summary$k_studies,
            res_overall$summary$k_cohorts),
    sprintf("Cluster-robust 95%% CI %.2f-%.2f by cohort",
            res_overall$summary$SMR_LCI_robust,
    ...
    "figures_combined.pdf and figures_combined_period_sorted.pdf, plus PNG copies in the figures folder."
  )
)
writeData(wb, "README", readme)
addStyle(wb, "README", header_style, rows = 1, cols = 1:ncol(readme),
         gridExpand = TRUE)
...
for (sheet_name in names(result_tables)) {
  ...
  setColWidths(wb, sheet_name,
               cols = 1:ncol(result_tables[[sheet_name]]), widths = "auto")
}
...
cat("  Cluster-robust 95% CI: ",
    sprintf("%.2f-%.2f",
            res_hic_ukpds$summary$SMR_LCI_robust,
